...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
	...
	data->fontIcons = nvgCreateFont(vg, "icons", PATH "/entypo.ttf");
	if (data->fontIcons == -1) {
		printf("Could not add font icons.\n");
		return -1;
	}
	data->fontNormal = nvgCreateFont(vg, "sans", PATH "/Roboto-Regular.ttf");
	if (data->fontNormal == -1) {
		printf("Could not add font italic.\n");
		return -1;
	}
	data->fontBold = nvgCreateFont(vg, "sans-bold", PATH "/Roboto-Bold.ttf");
	if (data->fontBold == -1) {
